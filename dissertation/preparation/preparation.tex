...
First, we modify the graph such that the longest path is always connected to the 
...
I selected execution traces from several GNU coreutils programs (v8.32) to benchmark the analysis tool, all executed on 
an AArch64 architecture.

\subsection{Dataset listing} \label{prep:datasets}
...
%TODO: Verbatim
Before exploring the properties of the chosen datasets, we introduce basic terminology for distinguishing various types 
of repeating patterns \cite{PMRepeats}. Repeating substrings (or \textit{repeats}) in a string can be
\begin{itemize}
