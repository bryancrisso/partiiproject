...
\begin{tabular}{lcccccc}
  ...
  \cmidrule(l){4-7}
  & (Total) & (Total) & Period\protect\footnotemark & ACR\protect\footnotemark & TI\protect\footnotemark & Coverage\\
  \midrule
...
window size of 5000 and a history size of 200.}
\footnotetext[2]{Period is measured in length of basic blocks and is the length of one iteration of a pattern.}
\footnotetext[3]{Average Consecutive Repetitions: The mean number of times a period of a pattern will repeat without interruption.}
\footnotetext[4]{Total Iterations: The total number of times a period of a pattern exists in the program trace.}

As shown in the table, all datasets exhibit a `largest pattern' covering a significant proportion of the trace.
The datasets additionally have other significant patterns not shown in the table.
...
This section details the software engineering approaches used during the project's implementation, specification of the 
project requirements, and an overview and justification of the tools used.

\subsection{Software engineering}

% The use of AI tools for background research and general subject understanding is always allowed and
% does not have to be declared. However, for any use of AI tools, it should be remembered that they
% may “hallucinate” details or make factual errors, while producing very believable text, and they may
% also reproduce others’ content without acknowledgement. Good academic standards require fact
% checking for correctness, and where appropriate, citation of suitable sources for any information
% presented within assessed work.

I used the spiral methodology when implementating the system, iterating between planning, implementing, and evaluating 
a new feature.

Code was regularly committed and pushed to GitHub, using feature branches to implement large changes.
...
Show that I can get valid longest paths using matrix approach

Show that I can achieve performance parity in terms of wall time by exploiting patterns

Explore different strategies for pattern exploitation

Explore different strategies for matrix representation multiplication

Show that the matrix representation is formulated of two independent sub-problems by de-synchronising them.

Demonstrate parallelism of underlying representation by introducing multithreaded compute on both sides of the problem.

Attempt to use accelerator (GPU) compute to speed up matrix multiplications.

\subsection{Project tooling}

Programming language: C++ - allowing for fine-grained control over the underlying hardware (including memory management
and multithreading), which is particularly important in a performance sensitive project.

Additional compute frameworks: OpenCL - while other proprietary frameworks such as CUDA offer improved performance, the 
cross-platform nature of the framework allows me to iterate and test code locally on my Intel-based laptop while also being able 
to run benchmarks on more powerful Nvidia-based systems.
