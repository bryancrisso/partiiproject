...
I selected execution traces from several GNU coreutils programs (v8.32) \cite{coreutils} to benchmark the analysis tool, all executed on 
an AArch64 architecture. Execution traces are lists of the instructions executed during an execution of a program.
In our case, the execution traces are given as a series of branches, which can be interpreted as a series of basic 
blocks. A basic block is a series of instructions executed sequentially without a branch.
...
\begin{itemize}
  ...
  calculating the CRC of a 1MiB file of random bytes.

  \item \verb|cksum < urandom_32MiB.bin|

  the {\color{blue}\textbf{\href{https://www.gnu.org/software/coreutils/manual/html\_node/cksum-invocation.html}{coreutils cksum utility}}} ,
  calculating the CRC of a 32MiB file of random bytes.
  ...
  factorising 9696593808888933181586.
  ...
  computing the SHA1 message digest of a 1MiB file of random bytes.
  ...
  sorting a 2000 line text file of hexadecimal digits encoding random bytes.
  ...
  shuffling a 34953 line text file of hexadecimal digits encoding random bytes.
\end{itemize}
...
Repeating substrings (or \textit{repeats}) in a string can be
\begin{itemize}
  \item \textit{tandem} repeats where successive repeat substrings immediately follow each other. They characterise 
    loops in a program trace.
  \item \textit{split} repeats where repeat substrings are separated by other symbols. These can characterise repeated 
    function calls where each call has the same dependency information.
  \item \textit{overlapping} repeats where repeat substrings overlap.
\end{itemize}

We can further refine the terminology for tandem repeats. A tandem repeat is \textit{primitive} if it is itself 
not composed of tandem repeats of another substring. A set of tandem repeats is \textit{maximal} if there is no identical 
substring immediately preceding or succeeding the sequence of tandem repeats. We refer to primitive and maximal tandem 
