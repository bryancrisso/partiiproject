...
register-only longest chains (\ref{chap:intro:section:register-only-chains})
and alternative graph structures (\ref{chap:prep:alternative-graph-structure})
...
Now that we have a method for finding the longest path in a directed acyclic graph, we have to construct the graph 
upon which to perform our method. Let us first consider the dependencies for instructions only affecting registers.
Since each instruction has a fixed set of registers which it accesses, it has a fixed dependency structure, and so each 
instruction in the program trace will have a fixed subgraph in the DDG encoding its dependency information.
...
\begin{itemize}
  ...
  \item The values in \verb|R1| and \verb|R2| are unchanged by the instruction, so their respective states have 0-cost 
    edges between them. This connects dependency chains from the source registers to any dependency chains starting 
    at the destination, without increasing their length.
...
The full DDG will consist of several of these subgraphs, horizontally glued together, one new set of dependency edges 
...
This defines the shape of the data that we will be working on as a mathematical expression consisting of a series of matrix
multiplications.
