...
execute one instruction, in program order, usually through a pipeline architecture.
...
\begin{itemize}
  \item Branch/jump mispredictions, which trigger pipeline flushes that require reissue of instructions.
...
provides the program's path, the simulator does not need to compute the results of instructions, and so it purely acts 
as a dependency graph analyser and critical path calculator.
...
and try to find the theoretical limit of ILP available in a program, or instead introduce realistic bounds, such as
...
compute the longest path in the DDG. This approach becomes a problem of reducing a large equation of matrix multiplications.

The immediately obvious benefit is that the matrix equation is a divisible computation, due to the associativity of 
...
One additional benefit of the divisibility of the matrix equation is that it makes instruction window scaling for 
