...
Modern superscalar processors commonly exploit ILP to increase throughput and hide latency \cite{shen2013modern}. The absolute limit of 
...
%TODO: Extrapolate naive baseline.
Traditionally, limit studies compute this longest path using a single-pass sequential sweep over the execution trace, and 
at each step calculating the length of dependency paths ending at each register. At the end of the trace, it picks out the 
longest path~\cite{wall-ilp, ilp-spec95, perpi}. Because the algorithm traverses the trace sequentially to propagate
...
benchmark suites under various parameters, such as restricted instruction windows or different branch prediction models.
...
I show experimentally that a single-threaded pipeline equipped with the data-driven optimisations matches the performance 
of the naive baseline. Furthermore, by distributing expression reduction and matrix multiplication workloads over 
multiple cores, the parallel approach outperforms the baseline, demonstrating the scalability of the matrix-based approach 
in analysing large execution traces.
