...
function enqueueMultiplication(MatrixJob newJob) {
  ...
  newJob.C.generatingJob = newJob;

  checkDependency(A,C);
  checkDependency(B,C);

  if (newJob.unresolvedDeps == 0) {
    readyJobs.push(newJob);
  }
}
\end{lstlisting}

\begin{lstlisting}[language=C++, caption={Dependency Processing}, label={alg:deps1}]
// FAS: Atomic fetch and subtract

function checkDependency(Matrix* dependency, Matrix* dependent) {
  MatrixJob job = dependency.generatingJob;

  if (job != nullptr
   && job.dependents.insert(dependent.generatingJob)) {
    return;
  }

  FAS(newJob.unresolvedDeps, 1);
}
\end{lstlisting}
...
void executeMultiplication(MatrixJob job) {
  *(job.C) = *(job.A) * *(job.B);
  ...
  Node<T> head = job.dependents.close();

  /* traverses the dependents list,
     decrementing unresolvedDeps and queueing a job if ready. */
  notifyAndQueueDependents(head);

  job.C.generatingJob = null;
}
    \end{lstlisting}
...
Node<T> close() {
  ...
  return tmp;
    ...
    \end{lstlisting}
    \caption{Lock-free list operations.}
    \label{alg:lock-free-list}
\end{algfloat}
